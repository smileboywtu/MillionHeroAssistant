...
def kwquery(query,answers):
    ...
    # 抓取百度前10条的摘要
    soup_baidu = To.get_html_baidu('https://www.baidu.com/s?wd=' + parse.quote(query))
    ...
    # 获取bing的摘要
    soup_bing = To.get_html_bing('https://www.bing.com/search?q=' + parse.quote(query))
    # 判断是否在Bing的知识图谱中
    ...
    if flag == 0:
        ...
        sorted_lists = sorted(target_list.items(), key=lambda item: item[1], reverse=True)
